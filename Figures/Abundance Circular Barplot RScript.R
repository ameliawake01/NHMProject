...
#ggplot(abundancedata, aes(fill=Species, y=Percentage, x=Condition)) + 
...
p <- ggplot(abundancedata, aes(x=as.factor(ID), y=Percentage, fill=Condition)) +
  ...
  geom_text(data=base_data, aes(x = title, y = -18, label=Condition), hjust=c(1,1,0,0), colour = "black", alpha=0.8, size=4, fontface="bold", inherit.aes = FALSE)
