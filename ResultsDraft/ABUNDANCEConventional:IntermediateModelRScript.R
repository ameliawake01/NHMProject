########################################################################################################
#############ABUNDANCE MODEL SCRIPT: CONVENTIONAL:INTERMEDIATE##########################################
########################################################################################################
...
calc.effect <- function(siteone, sitetwo) {
  ...
  if (siteone$Management == "Conventional" && sitetwo$Management == "Intermediate"){
    n1i <- siteone$n1i
...
#Running the model again with moderators (this wont work as there is only one soil type in such a small dataset)
...
#Subsetting by species (No abundance data for eukaryotes)
