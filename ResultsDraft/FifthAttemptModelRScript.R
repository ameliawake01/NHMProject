...
#Constructing the model for the bacterial subset
funghi.model <- rma.mv(yi=funghi_subset$LRR,
                         V=funghi_subset$LRR_var,
                         random=list(~ 1 | ID),
                         slab=ID,
                         data=funghi_subset)
summary(funghi.model)

#Making a forest plot of the bacteria model
forest(funghi.model)

#Pooling estimates and variances from the model to see overall effect
estimates <- c(coef(funghi.model))
variances <- c(vcov(funghi.model))

#Making a forest plot containing estimates and variances for the bacteria model
forest(estimates, variances, slab="Pooled effect")

#Constructing the model for the bacterial subset
eukaryotes.model <- rma.mv(yi=eukaryote_subset$LRR,
                         V=eukaryote_subset$LRR_var,
                         random=list(~ 1 | ID),
                         slab=ID,
                         data=eukaryote_subset)
...
#Making a forest plot containing estimates and variances for the bacteria model
forest(estimates, variances, slab="Pooled effect")f